      ...
      <div class="groupSelect">
        <div class="left">
          ...
          <el-select v-model="groupValue" class="ele-select" placeholder="请选择分组" @change="changeSelectName">
            <el-option v-for="item in selectGroup" :key="item.groupId" :label="item.groupName" :value="item.groupId">
            </el-option>
          </el-select>
        </div>
        <div class="right">
          <el-button v-if="isTrue == 'true' || isTrue == true" class="addNewGroup ele-btn" type="primary"
            @click="addNewGroup">新建分组</el-button>
          <el-button v-if="isTrue == 'true' || isTrue == true" class="addNewGroup ele-btn" type="primary"
            @click="deleteGroup">删除分组</el-button>
          <el-button v-if="isTrue == 'true' || isTrue == true" class="delGroupSelecet ele-btn" type="primary"
            @click="editorGroup">编辑分组</el-button>
          <el-button v-if="isTrue == 'true' || isTrue == true" class="uploadToUrl ele-btn" type="primary"
            @click="uploadImgToSystem">导入</el-button>
          <el-button v-else-if="isTrue == 'false' || isTrue == false" :disabled="isDisabled" class="uploadToUrl ele-btn"
            type="primary" @click="routerQuery">确认</el-button>
        </div>
      </div>
      <div class="main">
        <div class="p-top-nav">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            ...
            <div>
              <el-col class="uoloadImgCard" :span="8" v-for="(item, index) in imgsList" :key="index"
                :offset="index > 0 ? 1 : 1">
                <el-checkbox v-if="id == '1'" class="ChoiceImg" :checked="item.checked" v-model="item.checked"
                  @change="changeImg(item.materialPath,item.materialSmallPath,groupUrl,index,item.checked,item.materialType,item.materialWidth,item.materialHeight,item.materialSize)">
                </el-checkbox>
                <el-card :body-style="{ padding: '0px'}" shadow="never">
                  <div style="width:200px;height:200px;overflow:hidden;">
                    <img :src="groupUrl+item.materialSmallPath" class="image"
                      @click="cardCheck(item.materialPath,item.materialSmallPath,groupUrl,index,item.checked,item.materialType,item.materialWidth,item.materialHeight,item.materialSize)">
                  </div>
        ...
        <div class="pagination" v-if="imgsList.length!=0||videoList.length!=0">
          <el-pagination @current-change="handleCurrentChange" :current-page.sync="page" :page-size="currentPage1"
            layout="total, prev, pager, next" :total="total">
          </el-pagination>
        </div>
        <!--上传-->
        <el-dialog title="图片上传" :visible.sync="dialogVisible" @close="close" width="30%">
          ...
          <el-upload ref="uploadImg" class="upload-demo" action="111" :http-request="httpUpLoadRquest"
            :before-upload="beforeUpLoadImg" :on-progress="uploadVideoProcess" :on-success="successUpload"
            :on-error="errEve" :on-remove="removeUpload" multiple :limit="999	" :auto-upload="false" list-type="picture"
            :file-list="fileList">
            ...
            <el-select v-model="groupUploadValue" class="ele-select" placeholder="请选择">
              <el-option v-for="item in uploadGroup" :key="item.groupId" :label="item.groupName" :value="item.groupId">
              </el-option>
            </el-select>
            <div slot="tip" class="el-upload__tip upload-tip">只能上传图片格式jpg/png文件<br />图片不能超过2M,请勿修改图片后缀名,否则发送朋友圈可能失败
            </div>
          </el-upload>
        ...
        <!-- 新建 -->
        <el-dialog title="新建分组" :visible.sync="newAddGroup" width="600px" style="margin-top:15vh;"
          :close-on-click-modal="false" :show-close="false" center>
          <p style="text-align:center;"><em>分组名称：</em>
            <el-input class="ele-input" v-model="addNewgroupName" placeholder="请输入分组名称"></el-input>
          </p>
          <span slot="footer" class="dialog-footer">
            <el-button @click="newAddGroup = false,addNewgroupName=''" class="ele-btn">取 消</el-button>
            <el-button type="primary" @click="confirmHandle" class="ele-btn">确 定</el-button>
          </span>
        </el-dialog>
        <!-- 删除分组 -->
        <el-dialog title="删除分组" :visible.sync="showDeleteGroup" width="600px" style="margin-top:15vh;"
          :close-on-click-modal="false" :show-close="false" center>
          <p style="text-align:center;"><em>分组名称：</em>
            <el-select v-model="deleteGroupValue" class="ele-select" placeholder="请选择分组">
              <el-option v-for="item in selectGroup" :key="item.groupId" :label="item.groupName" :value="item.groupId"
                :disabled='item.groupName=="默认"'>
              </el-option>
            </el-select>
          </p>
          <span slot="footer" class="dialog-footer">
            <el-button @click="showDeleteGroup = false,deleteGroupValue=''" class="ele-btn">取 消</el-button>
            <el-button type="primary" @click="handleDeleteGroup" class="ele-btn">确 定</el-button>
          </span>
        </el-dialog>
        <!-- 编辑 -->
        <el-dialog title="编辑分组" :visible.sync="showEditorGroup" width="600px" style="margin-top:15vh;"
          :close-on-click-modal="false" :show-close="false" center>
          <p style="text-align:center;margin-bottom:20px;"><em>选择分组：</em>
            <el-select v-model="deleteGroupValue" class="ele-select" placeholder="请选择分组">
              <el-option v-for="item in selectGroup" :key="item.groupId" :label="item.groupName" :value="item.groupId"
                :disabled='item.groupName=="默认"'>
              </el-option>
          ...
          <p style="text-align:center;"><em>命名分组：</em>
            <el-input style="width:160px;height:30px;" v-model="editorGroupName" placeholder="请输入分组名称"></el-input>
          </p>
          <span slot="footer" class="dialog-footer">
            <el-button @click="showEditorGroup = false,deleteGroupValue=''" class="ele-btn">取 消</el-button>
            <el-button type="primary" @click="handleEditorGroup" class="ele-btn">确 定</el-button>
          </span>
        </el-dialog>
        <!-- 下载 -->
        <el-dialog title="下载" :visible.sync="showDownloadImg" width="600px" style="margin-top:15vh;" center>
          <p style="text-align:center;">
            确定要下载该素材吗？
          </p>
          <span slot="footer" class="dialog-footer">
            <el-button @click="showDownloadImg = false" class="ele-btn">取 消</el-button>
            <el-button type="primary" @click="handleDownloadImg" class="ele-btn">确 定</el-button>
          </span>
        </el-dialog>
        <!-- 删除图片 -->
        <el-dialog title="删除" :visible.sync="showDeleteImg" width="600px" style="margin-top:15vh;" center>
          <p style="text-align:center;">
            确定要删除该素材吗？
          </p>
          <span slot="footer" class="dialog-footer">
            <el-button @click="showDeleteImg = false" class="ele-btn">取 消</el-button>
            <el-button type="primary" @click="handleDeleteImg" class="ele-btn">确 定</el-button>
          </span>
        </el-dialog>
        <!-- 修改分组	 -->
        <el-dialog title="修改分组" :visible.sync="showAlterGroup" width="600px" style="margin-top:15vh;"
          :close-on-click-modal="false" :show-close="false" center>
          <p style="text-align:center;"><em>分组名称：</em>
            <el-select v-model="alterGroupValue" class="ele-select" placeholder="请选择分组">
              <el-option v-for="item in selectGroup" :key="item.groupId" :label="item.groupName" :value="item.groupId">
              </el-option>
            </el-select>
          </p>
          <span slot="footer" class="dialog-footer">
            <el-button @click="showAlterGroup = false,alterGroupValue=''" class="ele-btn">取 消</el-button>
            <el-button type="primary" @click="handleAlterGroup" class="ele-btn">确 定</el-button>
          </span>
        </el-dialog>
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        deleteGroupValue: "",
        showEditorGroup: false,
        editorGroupName: "",
        showDownloadImg: false,
        showDeleteImg: false,
        showAlterGroup: false,
        alterGroupValue: ""
      };
    },
    computed: {
    ...
    mounted() {
      this.groupUrl = publicUrl.info().materTestUrl
      this.imgUrl = publicUrl.info().materTestUrl
    ...
    methods: {
      // 修改分组
      handleAlterGroup() {
        if (this.alterGroupValue == '') {
          this.$message({
            message: '请选择分组',
            type: 'warning'
          })
        } else {
          axios.get(this.groupUrl + 'file/update_group?materialId=' + this.groupMoveid + '&currentGroupId=' + this
              .alterGroupValue)
            .then(res => {
              if (res.status == 200) {
                this.$message({
                ...
                this.alterGroupValue = '';
                this.showAlterGroup = false;
      ...
      handleEditorGroup() {
        if (this.deleteGroupValue == '' || this.editorGroupName == '') {
          this.$message({
            message: '请填写正确信息',
            type: 'error'
          })
          ...
          let data = {
            "groupId": this.deleteGroupValue,
            "groupName": this.editorGroupName,
            "userId": -1
          }
          axios.post(this.groupUrl + 'edit_group', data).then(res => {
            if (res.data.code == 1) {
              ...
              this.showEditorGroup = false;
              this.getGroupselectList();
              this.deleteGroupValue = "";
              this.editorGroupName = "";
            } else {
              this.$message({
      ...
      handleDeleteGroup() {
        if (this.deleteGroupValue == "") {
          ...
          axios.get(this.groupUrl + 'del_group?userId=' + this.userID + '&groupId=' + this.deleteGroupValue).then(
          res => {
            if (res.status == 200) {
              this.$message({
                message: '删除成功',
      ...
      showModel(type, title, item) {
        ...
        switch (type) {
          ...
          case "delete":
            ...
            _this.deletePath = _this.groupUrl + 'file/del?materialId=' + item.materialId + '&materialName=' + item
              .materialSmallPath;
            break;
        }
      ...
      beforeUpLoadImg(file) { // 上传之前判断
        ...
        if (
          ...
          if (['video/mp4', 'audio/ogg', 'video/flv', 'video/avi', 'video/wmv', 'video/rmvb'].indexOf(file.type) == -
            1) {
      ...
      .p-top-nav {
        ...
        .navbar-form {
          ...
          .input-group {
            ...
            color: #555555;
            margin: 0 20px;
            background: url(/static/img/search.e7a0036.png) no-repeat 202px center;
