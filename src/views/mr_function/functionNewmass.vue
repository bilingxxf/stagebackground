				...
				<li class="checkbox_li">
					<template>
						...
						<el-checkbox-group v-model="selectGroups" @change="handleCheckedCitiesChange">
							<el-checkbox v-for="item in groups" :label="item.wx_contact_group_id" :key="item.wx_contact_group_id">{{item.wx_contact_group_name}}</el-checkbox>
						</el-checkbox-group>
					</template>
				</li>
			</ul>
	...
	export default {
		...
		mounted() {
			...
			if(JSON.parse(sessionStorage.getItem('resource'))){
				...
				console.log(JSON.parse(sessionStorage.getItem('resource')))
		...
		methods: {
			...
			getSendImg(){
				let length = this.groupSentInfo.length-1;
				  axios.get(
				          "http://106.14.222.244:8000/api/v1/usekey?type=jy_GroupsentMessageImg"+length+"&key=" +
				            sessionStorage.getItem("user_id")
				       ).then(res => {
				          if (res.data.code == 0) {
								let img = 'http://106.14.222.244/'+res.data.data;
								this.groupSentInfo[0].image=img;
							axios.post(
									"http://106.14.222.244:8000/api/v1/text?type=jy_GroupsentMessage&key=" + sessionStorage.getItem('user_id'),
									this.groupSentInfo, {
										headers: {
...
//	                });      
			...
			getGroupSentInfo(){
				 axios.get(
                    "http://106.14.222.244:8000/api/v1/usekey?type=jy_GroupsentMessage&key=" +
                        sessionStorage.getItem("user_id")
               ).then(res => {
                    if (res.data.code == 0) {
...
//                      console.log(this.groupSentInfo)
                        this.uploadUrl ="http://106.14.222.244:8000/api/v1/file?type=jy_GroupsentMessageImg"+this.groupSentInfo.length+"&key=" +sessionStorage.getItem("user_id");
                    } else {
                    }
                })
                .catch(err => {});
            ...
            console.log(data)
						data.result.forEach(item=>{
							this.temporary.push(item.wx_contact_id)
						})
