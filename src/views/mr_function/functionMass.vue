<template>
  <div id="friend">
    <div class="title">
      ...
      <div class="inp-btn">
        <div class="inp">
          <el-input placeholder="请输入群发内容" v-model="searchContent" @keyup.enter.native="getSearchContent">
             <el-button slot="append" icon="el-icon-search" @click="getSearchContent"></el-button>
          </el-input>
        </div>
        <div class="btn">
          <el-button type="primary" @click="addPlan">
            <i class="el-icon-plus el-icon--left"></i>添加分组</el-button>
        </div>
        <div class="tablefn">
...
export default {
  ...
  methods: {
    ...
    getGroupSentInfo() {
			 axios.get("http://106.14.222.244:8000/api/v1/usekey?type=jy_GroupsentMessage&key=" +
    ...
    findDetails(item) {
      axios
        .get(
          "http://106.14.222.244:8000/api/v1/usekey?type=tjy_friendMass&key=" + item,
          { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
        )
        .then(res => {
          if (res.data.code == 0) {
    ...
    deletePoint(index) {
    	this.tableData.splice(index,1)
			axios.post("http://106.14.222.244:8000/api/v1/text?type=jy_GroupsentMessage&key=" + sessionStorage.getItem('user_id'),
						this.tableData, {
							headers: {
								"Content-Type": "application/x-www-form-urlencoded"
							}
